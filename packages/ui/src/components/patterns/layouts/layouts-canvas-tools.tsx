...
export function DotGrid({ pan, zoom }: { pan: Point; zoom: number }) {
  const size = 24 * zoom;
  return (
    <div
      aria-hidden
...
export function SelectionFrame({ shape, zoom }: { shape: Shape; zoom: number }) {
  const handle = 8 / zoom;
...
export function ShapeIcon({ type }: { type: ShapeType }) {
  if (type === "ellipse")
